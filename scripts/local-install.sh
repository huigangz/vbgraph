#!/usr/bin/env bash
# Build the current branch and link it as the global `vbgraph` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global vbgraph -> $(command -v vbgraph)"
  exit 0
fi
...
LINKED=$(command -v vbgraph || echo "(not on PATH)")
echo
echo "✓ global vbgraph now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"
