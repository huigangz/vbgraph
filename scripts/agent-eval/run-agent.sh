#!/usr/bin/env bash
# Headless Claude Code run against a repo with vbgraph MCP, capturing the
...
# Env: AGENT_EVAL_OUT (default /tmp/agent-eval), CG_BIN (vbgraph dist binary)
set -uo pipefail

REPO="$1"; LABEL="$2"; PROMPT="$3"
CG_BIN="${CG_BIN:-$(command -v vbgraph || echo /usr/local/bin/vbgraph)}"
OUT_DIR="${AGENT_EVAL_OUT:-/tmp/agent-eval}"; mkdir -p "$OUT_DIR"
OUT="$OUT_DIR/run-${LABEL}.jsonl"

MCP_CONFIG=$(cat <<JSON
{"mcpServers":{"vbgraph":{"command":"${CG_BIN}","args":["serve","--mcp","--path","${REPO}"]}}}
JSON
)
