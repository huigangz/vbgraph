/**
 * Adaptive output budget for vbgraph_explore (#185).
 *
 ...
 */
describe('vbgraph_explore output respects the adaptive budget', () => {
  let testDir: string;
  let cg: VBGraph;
  let handler: ToolHandler;

  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vbgraph-explore-budget-'));
  ...
  it('keeps total output under the small-project cap', async () => {
    const result = await handler.execute('vbgraph_explore', { query: 'Session method helper' });
  ...
  it('prefixes source lines with line numbers by default (cat -n style)', async () => {
    delete process.env.VBGRAPH_EXPLORE_LINENUMS;
    const result = await handler.execute('vbgraph_explore', { query: 'Session method helper' });
    const text = result.content?.[0]?.text ?? '';
  ...
  it('omits line numbers when VBGRAPH_EXPLORE_LINENUMS=0', async () => {
    process.env.VBGRAPH_EXPLORE_LINENUMS = '0';
    try {
      const result = await handler.execute('vbgraph_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
