...
describe('supportsUnicode', () => {
  ...
  it('returns false on Windows by default (mojibake-prone consoles)', () => {
    withEnv({ VBGRAPH_ASCII: undefined, VBGRAPH_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('win32');
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ VBGRAPH_ASCII: undefined, VBGRAPH_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects VBGRAPH_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ VBGRAPH_UNICODE: '1', VBGRAPH_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects VBGRAPH_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ VBGRAPH_ASCII: '1', VBGRAPH_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('VBGRAPH_ASCII takes precedence over VBGRAPH_UNICODE', () => {
    withEnv({ VBGRAPH_ASCII: '1', VBGRAPH_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns ASCII glyphs on Windows', () => {
    withEnv({ VBGRAPH_ASCII: undefined, VBGRAPH_UNICODE: undefined }, () => {
      setPlatform('win32');
      const g = getGlyphs();
