 ...
 * reliably crashes VBGraph with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when VBGraph detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    sep,
    `[VBGraph] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    'Node.js 25.x has a V8 WASM JIT (turboshaft) Zone allocator bug that',
    'crashes with `Fatal process out of memory: Zone` when VBGraph',
    'compiles tree-sitter grammars. VBGraph WILL crash on this Node',
    ...
    '  VBGRAPH_ALLOW_UNSAFE_NODE=1 vbgraph ...',
    sep,
  ].join('\n');
}
