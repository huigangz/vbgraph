 ...
 *
 * Load, save, and validate VBGraph configuration.
 */

import * as fs from 'fs';
import * as path from 'path';
import picomatch from 'picomatch';
import { VBGraphConfig, DEFAULT_CONFIG, LANGUAGES, NodeKind } from './types';
...
export function getConfigPath(projectRoot: string): string {
  return path.join(projectRoot, '.vbgraph', CONFIG_FILENAME);
}
 ...
 */
export function validateConfig(config: unknown): config is VBGraphConfig {
...
function mergeConfig(
  defaults: VBGraphConfig,
  overrides: Partial<VBGraphConfig>
): VBGraphConfig {
  return {
    ...
    // .vbgraph/config.json are not silently dropped on load.
 ...
 */
export function loadConfig(projectRoot: string): VBGraphConfig {
  ...
  try {
    ...
    const merged = mergeConfig(DEFAULT_CONFIG, parsed as Partial<VBGraphConfig>);
 ...
 */
export function saveConfig(projectRoot: string, config: VBGraphConfig): void {
  const configPath = getConfigPath(projectRoot);
  ...
  const toSave = { ...config };
  delete (toSave as Partial<VBGraphConfig>).rootDir;
 ...
 */
export function createDefaultConfig(projectRoot: string): VBGraphConfig {
  return {
    ...DEFAULT_CONFIG,
    rootDir: projectRoot,
  };
 ...
 */
export function addIncludePatterns(projectRoot: string, patterns: string[]): VBGraphConfig {
 ...
 */
export function addExcludePatterns(projectRoot: string, patterns: string[]): VBGraphConfig {
  const config = loadConfig(projectRoot);
 ...
 */
export function shouldIncludeFile(filePath: string, config: VBGraphConfig): boolean {
