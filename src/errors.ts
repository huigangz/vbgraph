 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'vbgraph';
 *
...
/**
 * Base error class for all VBGraph errors.
 *
 * All VBGraph-specific errors extend this class, allowing you to catch
 * all VBGraph errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 *   await cg.indexAll();
 * } catch (error) {
 *   if (error instanceof VBGraphError) {
 *     console.log(`VBGraph error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
export class VBGraphError extends Error {
 ...
 */
export class FileError extends VBGraphError {
 ...
 */
export class ParseError extends VBGraphError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends VBGraphError {
 ...
 */
export class SearchError extends VBGraphError {
 ...
 */
export class VectorError extends VBGraphError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends VBGraphError {
...
/**
 * Simple logger for VBGraph operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.VBGRAPH_DEBUG) {
      console.debug(`[VBGraph] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[VBGraph] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[VBGraph] ${message}`, context ?? '');
  },
};
