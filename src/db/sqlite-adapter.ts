...
export function buildWasmFallbackBanner(nativeError?: string): string {
  ...
  const lines = [
    sep,
    '[VBGraph] WASM SQLite fallback active (better-sqlite3 unavailable)',
    sep,
    ...
    '',
    'Verify after fix: `vbgraph status` should show `Backend: native`.',
  ];
  if (nativeError) {
 ...
 * process (`tools.ts` getVBGraph cache), so a process-global would
 * race / overwrite.
 */
