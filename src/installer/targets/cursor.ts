 ...
 *     as Claude.
 *   - Instructions to `./.cursor/rules/vbgraph.mdc` (project-local
 ...
 * `workspaceFolders` in the MCP initialize call. The vbgraph MCP
 * server's `process.cwd()` fallback therefore misses the workspace's
 * `.vbgraph/` and reports "not initialized" on every tool call.
 *
...
function rulesPath(): string {
  return path.join(process.cwd(), '.cursor', 'rules', 'vbgraph.mdc');
}
...
const MDC_FRONTMATTER = [
  '---',
  'description: VBGraph MCP usage guide — when to use which tool',
  'alwaysApply: true',
  '---',
  '',
].join('\n');
...
class CursorTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    const alreadyConfigured = !!config.mcpServers?.vbgraph;
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const mcpPath = mcpJsonPath(loc);
    const config = readJsonFile(mcpPath);
    if (config.mcpServers?.vbgraph) {
      delete config.mcpServers.vbgraph;
    ...
    if (loc === 'local') {
      ...
      const action = removeMarkedSection(rules, VBGRAPH_SECTION_START, VBGRAPH_SECTION_END);
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { vbgraph: buildCursorMcpConfig(loc) } }, null, 2);
  ...
  /**
   * Write the project-local `.cursor/rules/vbgraph.mdc` file. Used
   * by `vbgraph init` to bootstrap projects that have only the
   * global `~/.cursor/mcp.json` — without the rules file, the Cursor
   * agent has no signal to prefer vbgraph over native grep.
   */
...
/**
 * Build the vbgraph MCP-server config for Cursor at the given
...
function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  const before = existing.mcpServers?.vbgraph;
  ...
  existing.mcpServers.vbgraph = after;
