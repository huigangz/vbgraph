...
export interface SyncResult {
  ...
  /**
   * Phase 3 (framework synthesize/augment) outputs. Optional so existing
   * SyncResult consumers compile without change. Callers that need the
   * count use `result.phase3?.nodesAdded ?? 0`.
   */
  phase3?: {
    nodesAdded: number;
    edgesAdded: number;
...
export class ExtractionOrchestrator {
  ...
  async indexAll(
    ...
    // Framework detection no longer happens here — Phase3Orchestrator
    // re-detects against the post-extraction DB (see src/resolution/phase3.ts).
    // The legacy per-file `extract` hook was removed in PR-16.
  ...
  async indexFileWithContent(
    ...
    // Extract from source. Framework synthesis (routes / components / DI
    // bindings / temporal dispatch) no longer happens here — it runs in
    // Phase 3 after extraction completes.
