 ...
 *   - `VBGRAPH_ASCII=1`  -> ASCII (escape hatch for any terminal)
 *   - `VBGRAPH_UNICODE=1` -> Unicode (opt-in on Windows)
...
export function supportsUnicode(): boolean {
  if (process.env.VBGRAPH_ASCII === '1') return false;
  if (process.env.VBGRAPH_UNICODE === '1') return true;
