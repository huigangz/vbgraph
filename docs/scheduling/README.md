# CodeGraph SCIP-refresh scheduling templates

Templates for running `codegraph scip-refresh` on a schedule. One template per
...
scheduling, you'd run `codegraph scip-refresh` manually whenever your
...
|---|---|---|
| macOS | [launchd.plist.template](launchd.plist.template) | `launchctl load -w ~/Library/LaunchAgents/com.codegraph.scip-refresh.plist` |
| Linux | [systemd.service.template](systemd.service.template) + [systemd.timer.template](systemd.timer.template) | `systemctl --user enable --now codegraph-scip-refresh.timer` |
| Windows | [task-scheduler.xml.template](task-scheduler.xml.template) | `schtasks /Create /XML codegraph-scip-refresh.xml /TN "CodeGraphScipRefresh"` |
...
1. Acquires the cross-process `FileLock` at `.codegraph/codegraph.lock`. If
   another `codegraph` process is indexing or refreshing, returns the
   `lock-failed` phase (exit code 1) without touching the database.
2. Spawns `config.scipRefreshCommand` (default `'scip-dotnet index ./'`).
3. Captures stdout/stderr to `.codegraph/logs/scip-refresh-<timestamp>.log`.
...
7. Writes `.codegraph/scip-last-refresh.json` with `refreshedAt`,
...
| 1 | Indexer failed (non-zero exit, crash, or missing output file) OR another `codegraph` process is holding the cross-process file lock. |
...
- **systemd** — `systemctl --user status codegraph-scip-refresh` shows "failed";
  `journalctl --user -u codegraph-scip-refresh -e` has the captured stderr.
...
`--quiet` scheduling, CodeGraph writes the same message to three places:
...
2. **Per-run log file** — `.codegraph/logs/scip-refresh-<timestamp>.log`
   gets a `[codegraph derived-data warning <iso>]` section appended.
   Persistent across runs.
3. **Sidecar** — `.codegraph/scip-last-refresh.json` includes a
   `lastError` field (string or null). `codegraph status` surfaces it
...
Override the spawn command per project in `.codegraph/config.json`:
...
```sh
codegraph scip-refresh --cmd "scip-typescript index" --scip-output ./out.scip
```
...
```sh
cat .codegraph/scip-last-refresh.json
```
...
- **Multi-indexer refresh** (running `scip-dotnet` + `scip-typescript` on the
  ...
  `codegraph index --scip-auto` does at index time. P3 may add `--scip-auto`
  to `scip-refresh`.
- **Cross-platform installer** (`codegraph schedule install`). The plan ships
